...
export function isValidAzuraImage(record, listed = false) {
  ...
  if (typeof record.image !== "string" || !IMAGE_PATH.test(record.image) || record.image.includes("..") ||
  ...
  try {
    ...
    if (method === "GET") {
      if (!Array.isArray(payload?.images) || payload.images.length > 1000 ||
          !payload.images.every((record) => isValidAzuraImage(record, true))) {
