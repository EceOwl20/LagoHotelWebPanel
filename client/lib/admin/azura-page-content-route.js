...
function failure(error) {
  return json({ error: error.message || "Azura sayfası bağlantısı başarısız oldu." }, error.status || 502);
}
...
export function createAzuraPageContentHandlers(pageKey) {
  if (!["spawellness", "spor", "beachpools", "kidsclub", "bars", "entertainment"].includes(pageKey)) throw new Error("Unsupported page");
  async function GET() {
  ...
  async function PUT(request) {
    ...
    const rateLimit = consumeRateLimit({
      key: `admin-write:azura-${pageKey}-page:${getClientIp(request)}`, limit: 30, windowMs: 60 * 1000,
    });
