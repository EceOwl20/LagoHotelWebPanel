...
import {
  ...
  heartbeatGalleryCategoryEditLock,
  releaseGalleryCategoryEditLock,
  serializeGalleryCategoryEditLock,
...
function lockErrorResponse(error, session) {
  return NextResponse.json(
    {
      error: error.message || "Galeri düzenleme kilidi işlemi tamamlanamadı.",
      lock: error.lock
        ? serializeGalleryCategoryEditLock(error.lock, session)
        : null,
    },
...
export async function POST(request, { params }) {
  ...
  const rateLimit = consumeRateLimit({
    key: `admin-gallery-edit-lock:${session.userId}:${getClientIp(request)}`,
    limit: 40,
    windowMs: 5 * 60 * 1000,
  });
  ...
  try {
    ...
    if (action === "release") {
      const released = releaseGalleryCategoryEditLock(
        categoryId,
        session,
        lockToken
      );
    ...
    if (action === "heartbeat") {
      const lock = heartbeatGalleryCategoryEditLock(
        categoryId,
        session,
        lockToken
      );
      return NextResponse.json({
        lock: serializeGalleryCategoryEditLock(lock, session),
    ...
    if (!/^[0-9a-f-]{36}$/i.test(clientId)) {
      return NextResponse.json(
        { error: "Geçersiz editör sekmesi kimliği." },
        { status: 400 }
      );
    }
    ...
    const lock = acquireGalleryCategoryEditLock(categoryId, session, {
      clientId,
