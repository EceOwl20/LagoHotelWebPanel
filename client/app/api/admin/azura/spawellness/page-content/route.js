...
function failure(error) {
  return json({ error: error.message || "Azura Spa & Wellness sayfası bağlantısı başarısız oldu." }, error.status || 502);
}
...
export async function GET() {
  ...
  try {
    ...
    return json(await requestAzuraSpaPage("GET"));
...
export async function PUT(request) {
  ...
  const rateLimit = consumeRateLimit({
    key: `admin-write:azura-spawellness-page:${getClientIp(request)}`, limit: 30, windowMs: 60 * 1000,
  });
  ...
  try {
    ...
    if (!body || typeof body !== "object" || Array.isArray(body) ||
        ...
        !isValidAzuraRevision(body.revision) || !isValidAzuraSpaPage(body.bundle, body.media)) {
      return json({ error: "Azura Spa & Wellness sayfası içeriği veya sürümü geçersiz." }, 400);
    }
    return json(await requestAzuraSpaPage("PUT", body.bundle, body.media, { revision: body.revision }));
  } catch (error) { return failure(error); }
}
