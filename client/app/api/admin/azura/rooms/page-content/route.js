...
import { isValidAzuraRoomsPage, requestAzuraRoomsPage } from "@/lib/admin/azura-rooms-page-content.mjs";
...
function failure(error) {
  return json({ error: error.message || "Azura oda sayfası bağlantısı başarısız oldu." }, error.status || 502);
}
...
export async function GET() {
  ...
  try {
    ...
    return json(await requestAzuraRoomsPage("GET"));
...
export async function PUT(request) {
  ...
  const rateLimit = consumeRateLimit({
    key: `admin-write:azura-rooms-page:${getClientIp(request)}`, limit: 30, windowMs: 60 * 1000,
  });
  ...
  try {
    ...
    if (!body || typeof body !== "object" || Array.isArray(body) ||
        ...
        !isValidAzuraRevision(body.revision) || !isValidAzuraRoomsPage(body.bundle, body.media)) {
      return json({ error: "Azura oda sayfası içeriği veya sürümü geçersiz." }, 400);
    }
    return json(await requestAzuraRoomsPage("PUT", body.bundle, body.media, { revision: body.revision }));
  } catch (error) { return failure(error); }
}
