...
import { isValidAzuraAboutPage, requestAzuraAboutPage } from "@/lib/admin/azura-about-page-content.mjs";
...
function failure(error) {
  return json({ error: error.message || "Azura Hakkımızda sayfası bağlantısı başarısız oldu." }, error.status || 502);
}
...
export async function PUT(request) {
  ...
  const rateLimit = consumeRateLimit({
    key: `admin-write:azura-about-page:${getClientIp(request)}`, limit: 30, windowMs: 60 * 1000,
  });
  ...
  try {
    ...
    if (!body || typeof body !== "object" || Array.isArray(body) ||
        ...
        !isValidAzuraRevision(body.revision) || !isValidAzuraAboutPage(body.bundle, body.media)) {
      return json({ error: "Azura Hakkımızda sayfası içeriği veya sürümü geçersiz." }, 400);
    }
    return json(await requestAzuraAboutPage("PUT", body.bundle, body.media, { revision: body.revision }));
  } catch (error) { return failure(error); }
}
