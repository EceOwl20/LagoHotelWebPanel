...
import { isValidExperienceText, requestAzuraExperienceText } from "@/lib/admin/azura-experience-text.mjs";
...
function failure(error) {
  return json({ error: error.message || "Azura metin bağlantısı başarısız oldu." }, error.status || 502);
}
...
export async function GET() {
  ...
  try {
    ...
    return json(await requestAzuraExperienceText("GET"));
  } catch (error) {
...
export async function PUT(request) {
  ...
  const rateLimit = consumeRateLimit({
    key: `admin-write:azura-experience-text:${getClientIp(request)}`,
    limit: 30,
    windowMs: 60 * 1000,
  });
  ...
  try {
    ...
    if (!body || typeof body !== "object" || Array.isArray(body) ||
        !Object.hasOwn(body, "experienceText") ||
        Object.keys(body).some((key) => !["experienceText", "revision"].includes(key)) ||
        (body.revision !== undefined && body.revision !== null && !isValidAzuraRevision(body.revision)) ||
        !isValidExperienceText(body.experienceText)) {
      return json({ error: "Azura tanıtım metinleri geçersiz." }, 400);
    }
    return json(await requestAzuraExperienceText("PUT", body.experienceText, { revision: body.revision ?? null }));
  } catch (error) {
    return failure(error);
  }
}
