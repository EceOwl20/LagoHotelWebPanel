...
export async function GET() {
  ...
  try {
    ...
    return json({ images: await requestAzuraImages("GET") });
  } catch (error) {
...
export async function POST(request) {
  ...
  const rateLimit = consumeRateLimit({
    key: `admin-upload:azura-experience:${getClientIp(request)}`,
    limit: 10,
    windowMs: 60 * 1000,
  });
  ...
  try {
    ...
    return json(await requestAzuraImages("POST", file), 201);
  } catch (error) {
    return failure(error);
  }
}
