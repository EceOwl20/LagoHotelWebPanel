...
function failure(error) {
  return json({ error: error.message || "Azura bölüm bağlantısı başarısız oldu." }, error.status || 502);
}
...
export async function GET(_request, { params }) {
  ...
  try {
    ...
    return json(await requestAzuraHomepageSection("GET", sectionKey));
  } catch (error) {
...
export async function PUT(request, { params }) {
  ...
  const rateLimit = consumeRateLimit({
    key: `admin-write:azura-homepage-section:${sectionKey}:${getClientIp(request)}`,
    limit: 30,
    windowMs: 60 * 1000,
  });
  ...
  try {
    ...
    if (!body || typeof body !== "object" || Array.isArray(body) ||
        ...
        !Object.hasOwn(body, "section") || !Object.hasOwn(body, "revision") ||
        !isValidAzuraRevision(body.revision) ||
        !isValidAzuraHomepageSection(sectionKey, body.section)) {
      return json({ error: "Azura bölüm içeriği veya sürümü geçersiz." }, 400);
    }
    return json(await requestAzuraHomepageSection("PUT", sectionKey, body.section, { revision: body.revision }));
  } catch (error) {
    return failure(error);
  }
}
