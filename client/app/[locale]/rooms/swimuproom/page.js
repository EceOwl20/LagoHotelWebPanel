...
const Page = async ({ params }) => {
  ...
  const [t, t2, t3, t4, media, sharedMedia] = await Promise.all([
    getTranslations("SwimupRoom"),
    getTranslations("SwimupRoom.RoomInfo"),
    getTranslations("SwimupRoom.RoomTour"),
    getTranslations("SwimupRoom.BackgroundSection"),
    readSitePageContent("swimuproom"),
  ...
  return (
    <div className=' overflow-hidden flex flex-col items-center justify-center gap-[60px] md:gap-[80px] lg:gap-[100px] bg-[#fbfbfb]'>
     ...
     <SubroomCarousel images={carouselImages}/>
      ...
      <RoomTour span={t3("subtitle")} header={t3("title")} text={t3("text")} link="https://kuula.co/share/collection/7bygW?logo=1&info=0&fs=1&vr=1&autorotate=0.22&autop=10&autopalt=1&thumbs=4&margin=2&alpha=0.72"/>
      <RoomTour span={t3("subtitle2")} header={t3("title2")} text={t3("text2")} link="https://kuula.co/share/collection/7bwmJ?logo=1&info=0&fs=1&vr=1&autorotate=0.22&autop=10&autopalt=1&thumbs=4&margin=2&alpha=0.72"/>
      <OtherOptions images={otherOptionImages} translationNamespace="SwimupRoom.OtherOptions"/>
      <ContactSection2/>
    </div>
  )
}
