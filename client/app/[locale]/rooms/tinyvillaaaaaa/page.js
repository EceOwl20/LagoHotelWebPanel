...
const Page = async ({ params }) => {
  ...
  const [t, t2, t3, t4, media, sharedMedia] = await Promise.all([
    getTranslations("TinyVilla"),
    getTranslations("TinyVilla.RoomInfo"),
    getTranslations("TinyVilla.RoomTour"),
    getTranslations("TinyVilla.BackgroundSection"),
    readSitePageContent("tinyvilla"),
  ...
  return (
    <div className=' overflow-hidden flex flex-col items-center justify-center gap-[60px] md:gap-[80px] lg:gap-[100px] bg-[#fbfbfb]'>
     ...
     <SubroomCarousel images={carouselImages}/>
      ...
      <OtherOptions images={otherOptionImages} translationNamespace="TinyVilla.OtherOptions"/>
      <ContactSection2/>
    </div>
  )
}
