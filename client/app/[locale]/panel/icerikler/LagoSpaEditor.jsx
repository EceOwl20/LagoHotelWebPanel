...
import SpaPageFields from "./SpaPageFields";

const BUNDLE_URL = "/api/admin/messages/namespace?namespace=Spa";
const MEDIA_URL = "/api/admin/site-pages/spawellness";
const same = (a, b) => JSON.stringify(a) === JSON.stringify(b);
const sameMedia = (a, b) => ["hero", "info", "gallery", "massage", "types"]
  .every((key) => same(a?.[key], b?.[key]));

const LagoSpaEditor = forwardRef(function LagoSpaEditor({
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!bundleResponse.ok) throw new Error(bundlePayload.error || "Lago Spa & Wellness metinleri alınamadı.");
        if (!mediaResponse.ok) throw new Error(mediaPayload.error || "Lago Spa & Wellness görselleri alınamadı.");
        if (!bundlePayload.bundle || !mediaPayload.content) throw new Error("Lago Spa & Wellness içeriği eksik.");
        if (!cancelled) {
  ...
  async function save() {
    ...
    if (!editable || !lockToken || saving) {
      setError("Spa & Wellness sayfası için düzenleme kilidi alınamadı veya kayıt sürüyor.");
      return "failed";
    }
    ...
    try {
      if (!same(draft.bundle, original.bundle)) {
        const response = await fetch("/api/admin/messages/namespace", {
          ...
          body: JSON.stringify({ namespace: "Spa", bundle: draft.bundle }),
        });
        const data = await response.json();
        if (!response.ok) throw new Error(data.error || "Lago Spa & Wellness metinleri kaydedilemedi.");
      ...
      if (!same(draft.media, original.media)) {
        const response = await fetch(MEDIA_URL, {
          ...
          headers: {
            ...
            "X-Panel-Edit-Namespace": "Spa",
          },
          body: JSON.stringify({ content: draft.media }),
        });
        const data = await response.json();
        if (!response.ok) throw new Error(data.error || "Lago Spa & Wellness görselleri kaydedilemedi.");
      }
      const [bundleResponse, mediaResponse] = await Promise.all([
        fetch(BUNDLE_URL, { cache: "no-store" }),
        fetch(MEDIA_URL, { cache: "no-store" }),
      ...
      setDraft(page);
      setNotice("Lago Spa & Wellness sayfası kaydedildi.");
      return "saved";
    } catch (cause) {
  ...
  return <div className="space-y-5">
    {loading ? <p className="rounded-2xl border border-stone-200 bg-white p-5 text-sm text-stone-500">Lago Spa & Wellness içerikleri yükleniyor...</p> : null}
