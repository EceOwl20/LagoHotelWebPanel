...
import RestaurantsPageFields from "../../icerikler/RestaurantsPageFields";
import { usePanelPermission } from "../../PanelSessionContext";
import { PANEL_PERMISSIONS } from "@/lib/admin/permissions.mjs";
import { isValidAzuraRestaurantsPage } from "@/lib/admin/azura-restaurants-page-content.mjs";
import { AZURA_RESTAURANT_MEDIA_FIELDS } from "@/lib/admin/restaurant-page-fields.mjs";

const API = "/api/admin/azura/restaurants/page-content";
const IMAGES = "/api/admin/azura/restaurants/images";
const same = (a, b) => JSON.stringify(a) === JSON.stringify(b);

const AzuraRestaurantsEditor = forwardRef(function AzuraRestaurantsEditor({ activeLocale, onDirtyChange }, ref) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!response.ok) throw new Error(data.error || "Azura restoran sayfası alınamadı.");
        if (!isValidAzuraRestaurantsPage(data.bundle, data.media)) {
          throw new Error("Azura restoran sayfası beklenen biçimde değil.");
        }
        if (!cancelled) {
    ...
    async function loadImages() {
      try {
        ...
        const data = await response.json();
        if (!response.ok) throw new Error(data.error || "Azura restoran görselleri alınamadı.");
        if (!Array.isArray(data.images)) throw new Error("Azura restoran görsel listesi geçersiz.");
  ...
  async function uploadImage(file) {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || "Azura restoran görseli yüklenemedi.");
  ...
  async function save() {
    ...
    if (!isValidAzuraRestaurantsPage(draft.bundle, draft.media)) {
      setError("Dört dilin tüm restoran metinleri ve 13 görselin yol, ölçü ve alt açıklamaları geçerli olmalıdır.");
      return "failed";
    }
    ...
    try {
      ...
      const data = await response.json();
      if (response.status === 409) throw new Error("Restoran sayfası başka bir sekmede değiştirildi. Değişikliklerinizi kopyalayıp sayfayı yenileyin.");
      if (!response.ok) throw new Error(data.error || "Azura restoran sayfası kaydedilemedi.");
      ...
      if (editVersion.current === version) {
        ...
        setSuccess("Azura restoran sayfası kaydedildi.");
  ...
  return <div className="space-y-5">
    <div className="rounded-2xl border border-stone-200 bg-white p-5 shadow-sm sm:p-6">
      <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-stone-400">Restoranlar sayfası</p>
      <h2 className="mt-1 text-xl font-semibold text-stone-900">Azura restoran içerikleri</h2>
      <p className="mt-2 text-sm leading-6 text-stone-500">Banner, tanıtım, iki üçlü kart grubu ve keşif alanı tek formda düzenlenir. Kart sırası sabittir.</p>
      {loading ? <p className="mt-4 text-sm text-stone-500">Azura restoran sayfası yükleniyor...</p> : null}
      {mediaLoading ? <p className="mt-4 text-sm text-stone-500">Azura restoran görselleri yükleniyor...</p> : null}
    ...
    {draft ? <RestaurantsPageFields bundle={draft.bundle} media={draft.media} activeLocale={activeLocale}
      mediaFields={AZURA_RESTAURANT_MEDIA_FIELDS} disabled={!canEdit || saving || uploading}
    ...
    {changed ? <p className="text-xs font-medium text-amber-700">Kaydedilmemiş restoran sayfası değişiklikleri var.</p> : null}
  </div>;
});

export default AzuraRestaurantsEditor;
